...
class teleop(Node):
    def __init__(self):
        ...
        self.srv = self.create_service(Grasp, 'grasp', self.grasp_callback)
    ...
    def joint_callback(self, msg):
        ...
        error = np.linalg.norm(np.array(self.now_joint) - np.array(self.target_joint))
        if error < 0.05:
            self.pos_bool = 0
        else:
            self.pos_bool = 1
        ...
        gripper_error = np.abs(msg.position[6] - gripper_target)
